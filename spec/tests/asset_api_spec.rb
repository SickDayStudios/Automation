...
describe "SceneFile Connections Test" do
	...
	describe "| Client: GK-Elite |" do
				...
				aggregate_failures "#{scene}" do
					...
					AssetAPI.scene_productoptions_keys(scene).each do |product|
						aggregate_failures "#{product}" do 
							...
							@manifest = JSON.parse(RestClient.get("http://madetoordertest.blob.core.windows.net/webgl/client/gk-elite/#{product}/" + manifest){|response, request, result| response })
							...
							expect(@models).to match_array(@values)
						end
					end
				end
			end
		...
		# 				puts ""
		# 				puts " - #{ENV['SITE']} | #{scene} | #{product}"
		# 				compare = AssetAPI.product_data_selection_values
		# 				product_handles = AssetAPI.product_handle_values(product)
		# 				manifest_keys = AssetAPI.manifest_parameter_keys(product,manifest)
		# 				mobile_manifest_keys = AssetAPI.manifest_parameter_keys(product,mobile_manifest)
		# 				manifest_values = AssetAPI.manifest_parameter_values(product,manifest)
		...
		# 		end
		# 	end
		# 	puts "KEY ::: Expected: Data to match Actual: Manifest"
		# 	puts "Missing = Not in Manifest"
		# 	puts "Extra = Not in Data"
		# end
	end
